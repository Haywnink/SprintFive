...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	SpeedMS := w.meanSpeed() * KmHInMsec // Правильная конвертация км/ч в м/с
	heightMtrs := w.Height / CmInM
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(SpeedMS, 2)/heightMtrs)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
